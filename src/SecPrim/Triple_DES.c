...
uint64_t des(uint64_t input, uint64_t key, char mode)
{
    ...
    L = (uint32_t)(init_perm_res >> 32) & L64_MASK; //  64 biti 32 bit 32 bit.
    ...
    C = (uint32_t)((permuted_choice_1 >> 28) & 0x000000000fffffff); // tablodaki bitler 28
    D = (uint32_t)(permuted_choice_1 & 0x000000000fffffff);         //ve kontrol
...
uint64_t triple_des_encrypt(uint8_t input[], uint64_t key[])
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        buffer = buffer | ((uint64_t)input[i] << ((8 - i - 1) * 8));
    }

    buffer = des(buffer, key[0], 'e');
    buffer = des(buffer, key[1], 'd');
    buffer = des(buffer, key[2], 'e');
...
uint64_t triple_des_decrypt(uint8_t input[], uint64_t key[])
{
    ...
    buffer = des(buffer, key[2], 'd');
    buffer = des(buffer, key[1], 'e');
    buffer = des(buffer, key[0], 'd');
