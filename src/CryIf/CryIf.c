...
extern Key_Element KE[NO_KEY_ELEMENT]; 
extern uint32 CDs_keys [NO_DRIVERS][NO_DRIVERS];  
...
if (FALSE==validateID(channelId) ){ //[SWS_CryIf_00028]
...
if (FALSE==validateID(channelId) ){//[SWS_CryIf_00130]
...
Std_ReturnType CryIf_Key_SetValid( uint32 cryIfKeyId){
	#if (CRYIF_DEV_ERROR_DETECT ==STD_ON)
if (FALSE==CryinitDone) { //[SWS_CryIf_00056]
Det_ReportError(MODULE_ID_CRYIF,0,CRYIF_KEY_SET_VALID_ID,CRYIF_E_UNINIT);
return E_NOT_OK;
}

if (FALSE==validateID(cryIfKeyId)) { //[SWS_CryIf_00057]
...
Std_ReturnType CryIf_KeyElementCopy( uint32 cryIfKeyId, uint32 keyElementId, uint32 targetCryIfKeyId, uint32 targetKeyElementId ){
  ...
  if (FALSE==CryinitDone) {//[SWS_CryIf_00110]
Det_ReportError(MODULE_ID_CRYIF,0,CRYIF_KEY_ELEMENT_COPY_ID ,CRYIF_E_UNINIT);
	
   return E_NOT_OK; 
}

if (FALSE==validateID(cryIfKeyId) ){//[SWS_CryIf_00111]
Det_ReportError(MODULE_ID_CRYIF,0,CRYIF_KEY_ELEMENT_COPY_ID, CRYIF_E_PARAM_HANDLE );
return E_NOT_OK;}

	if (FALSE==validateID(targetCryIfKeyId)){ //[SWS_CryIf_00112]
Det_ReportError(MODULE_ID_CRYIF,0,CRYIF_KEY_ELEMENT_COPY_ID, CRYIF_E_PARAM_HANDLE );
return E_NOT_OK;}
	
if (KE[keyElementId].size!=KE[targetKeyElementId].size){ //[SWS_CryIf_00115]
Det_ReportError(MODULE_ID_CRYIF,0,CRYIF_KEY_ELEMENT_COPY_ID, CRYIF_E_KEY_SIZE_MISMATCH );
	
	return E_NOT_OK;
}
	#endif
if(TRUE==check_Drivers(NO_OF_CHANNELS,NO_OF_CHANNELS,CDs_keys,cryIfKeyId,targetCryIfKeyId)){ //[SWS_CryIf_00113]
...
#if (CRYIF_DEV_ERROR_DETECT ==STD_ON)
  if (FALSE==CryinitDone) { //[SWS_CryIf_00068]
...
Std_ReturnType CryIf_KeyGenerate( uint32 cryIfKeyId){
   #if (CRYIF_DEV_ERROR_DETECT ==STD_ON)
 if (FALSE==CryinitDone) { //[SWS_CryIf_00073]
Det_ReportError(MODULE_ID_CRYIF,0,CRYIF_KEY_GENERATE_ID,CRYIF_E_UNINIT);
return E_NOT_OK; 
}
if (FALSE==validateID(cryIfKeyId) ){//[SWS_CryIf_00074]
...
#if (CRYIF_DEV_ERROR_DETECT ==STD_ON)
 if (FALSE==CryinitDone) { //[SWS_CryIf_00098]
Det_ReportError(MODULE_ID_CRYIF,0,CRYIF_CERTIFICATE_PARSE_ID,CRYIF_E_UNINIT);
return E_NOT_OK; 
}
if (FALSE==validateID(cryIfKeyId) ){ //[SWS_CryIf_00099]
...
#if (CRYIF_DEV_ERROR_DETECT ==STD_ON)
 if (FALSE==CryinitDone) { //[SWS_CryIf_00123]
Det_ReportError(MODULE_ID_CRYIF,0,CRYIF_CERTIFICATE_VERIFY_ID,CRYIF_E_UNINIT);
return E_NOT_OK; 
}
if (FALSE==validateID(cryIfKeyId) ){//[SWS_CryIf_00124]
Det_ReportError(MODULE_ID_CRYIF,0,CRYIF_CERTIFICATE_VERIFY_ID, CRYIF_E_PARAM_HANDLE );
return E_NOT_OK;}

if (FALSE==validateID(verifyCryIfKeyId) ){//[SWS_CryIf_00125]
Det_ReportError(MODULE_ID_CRYIF,0,CRYIF_CERTIFICATE_VERIFY_ID, CRYIF_E_PARAM_HANDLE );
return E_NOT_OK;}

if(FALSE==check_Drivers ( NO_OF_CHANNELS,NO_OF_CHANNELS,CDs_keys,cryIfKeyId,verifyCryIfKeyId)){ //[SWS_CryIf_00126]
Det_ReportError(MODULE_ID_CRYIF,0,CRYIF_CERTIFICATE_VERIFY_ID, CRYIF_E_PARAM_HANDLE);
return E_NOT_OK;}	
	
if ( NULL==verifyPtr)  {//[SWS_CryIf_00127]
